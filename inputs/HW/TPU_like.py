...
def memory_hierarchy_dut(multiplier_array):
    ...
    reg_W_128B = MemoryInstance(name="rf_128B", size=128 * 8, r_bw=8, w_bw=8, r_cost=0.095, w_cost=0.095, area=0, bank=1,
                             random_bank_access=False, r_port=1, w_port=1, rw_port=0, latency=1)

    reg_O_1K = MemoryInstance(name="rf_1KB", size=16, r_bw=16, w_bw=16, r_cost=0.54, w_cost=0.6, area=0, bank=1,
                              random_bank_access=False, r_port=2, w_port=2, rw_port=0, latency=1)

    ##################################### on-chip memory hierarchy building blocks #####################################

    sram_32KB_512_1r_1w = \
        MemoryInstance(name="sram_32KB", size=32768 * 8, r_bw=512, w_bw=512, r_cost=22.9, w_cost=52.01, area=0, bank=1,
    ...
    sram_2M_with_16_128K_bank_128_1r_1w = \
        MemoryInstance(name="sram_2MB", size=131072 * 16 * 8, r_bw=128 * 16, w_bw=128 * 16, r_cost=26.01 * 16, w_cost=23.65 * 16, area=0, bank=1,
                       random_bank_access=True, r_port=1, w_port=1, rw_port=0, latency=1, min_r_granularity=64, min_w_granularity=64)
...
def multiplier_array_dut():
    ...
    operand_spatial_sharing = {'I1': {(1, 0)},  # {'O': 'O', 'W': 'I2', 'I': 'I1'}
                               'I2': {(0, 0)},
                               'O': {(0, 1)},
                               }
